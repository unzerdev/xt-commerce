...
<h3><?php echo UnzerCw_Language::_('Capture'); ?></h3>
<form action="<?php echo $this->getActionUrl('capture', array('transaction_id' => $transaction->getTransactionId())); ?>" method="POST" class="table table-striped unzercw-line-item-grid" 
id="captureForm<?php echo $transaction->getTransactionId(); ?>">
	...
	<?php if ($transaction->getTransactionObject()->isPartialCapturePossible()):?>
		...
		<table class="table table-striped unzercw-form-grid" width="100%" border="0" cellspacing="0" cellpadding="6">
			...
			<tfoot>
				<tr>
					<td colspan="6" class="right"><?php echo UnzerCw_Language::_('Total Capture Amount'); ?>:</td>
		...
		<?php if ($transaction->getTransactionObject()->isCaptureClosable()):?>
			<div class="closable-box">
				<label for="close-transaction"><?php echo UnzerCw_Language::_('Close transaction for further captures'); ?></label>
